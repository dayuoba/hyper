...
/*
impl<T, B> Future for Http1AutoUpgrades<T, B>
where
    T: AsyncRead + AsyncWrite + Send + 'static,
    B: Payload + 'static,
{
    ...
    fn poll(&mut self) -> Poll<Self::Item, Self::Error> {
        let pending_upgrade = match self {
            Http1AutoUpgrades::Normal(ref mut h1) => {
                // Poll the inner connection normally, while looking for
                // an HTTP upgrade. If one happens, consumes the inner connection
                // to be able to fulfill the pending upgrade future.
                try_ready!(h1.poll_without_shutdown());

                // Getting here means the connection was 'Ready', or done-ish.
                // Check for a pending upgrade, and if it exists, fulfill it,
                // otherwise just poll shutdown.
                h1.pending_upgrade()
            },
            Http1AutoUpgrades::Closing(ref mut h1) => {
                return h1.poll_shutdown();
            },
            Http1AutoUpgrades::Upgraded => {
                // This shouldn't be reached, since once a connection
                // has been 'upgraded', this future returns Ok(Ready(()).
                // An executor *shouldn't* poll after a future is ready.
                unreachable!("polled after complete");
            }
        };

        let mut h1 = match mem::replace(self, Http1AutoUpgrades::Upgraded) {
            Http1AutoUpgrades::Normal(h1) => h1,
            _ => unreachable!("h1 state must be Normal"),
        };

        if let Some(pending) = pending_upgrade {
            let (io, _buf, _) = h1.into_inner();
            pending.fulfill(Upgraded::new(Box::new(io)));
            Ok(Async::Ready(()))
        } else {
            // With no upgrade, try to shutdown.
            //
            // Note: can't use try_ready!, since that'd return with NotReady
            // while the state is set to Upgraded...
            Ok(if h1.poll_shutdown()?.is_not_ready() {
                *self = Http1AutoUpgrades::Closing(h1);
                Async::NotReady
            } else {
                Async::Ready(())
            })
